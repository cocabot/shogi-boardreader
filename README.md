...
### ホーム画面版の全画面表示と盤を畳む操作
ホーム画面版は実際のウィンドウ高に追従し、Safariのツールバー相当の余白を残さず表示します。ステータスバー・ホームインジケータの安全余白は確保します。
分割バー左側の「盤を畳む」でPDFを広げ、「盤を戻す」で元の分割比率に戻せます。「盤を広く」でも再表示できます。畳んでも読み込み中の棋譜・盤面は保持します。
アイコンは深い青緑と明るい金属色の、将棋駒と本を組み合わせたデザインへ更新しました。ホーム画面のアイコンが切り替わらない場合は、Safariからホーム画面へ追加し直してください。


### ホーム画面アイコンの互換性
現行アイコンは駒の中央に本を配置した図柄です。iPhone向けにはサイズを明記した180pxのPNGを指定しています。以前の `icon-180.png` / `icon-modern-180.png` などのURLも配信を維持し、端末に残った旧HTMLから参照できるようにしています。すでに文字だけで追加されたアイコンは、オンラインで最新版に更新した後、Safariから追加し直してください。

ホーム画面版の高さは `100lvh` を基準にし、Safariのツールバー表示時の `innerHeight` が残った場合にも下端まで使用します。下の余白はホームインジケータ用の最大34pxに制限します。Safari内では引き続き表示中のビューポートに追従します。
